...
static void PrintMemInfo(void)
{
	...
	strcpy_s(infoBuf, INFO_BUFFER_SIZE, GetHumanSize(statex.ullAvailPhys, mem_human_sizes, 1024));
	printf("Physical memory (free/total): %s/%s\n", infoBuf,
		GetHumanSize(statex.ullTotalPhys, mem_human_sizes, 1024));
	strcpy_s(infoBuf, INFO_BUFFER_SIZE, GetHumanSize(statex.ullAvailPageFile, mem_human_sizes, 1024));
	printf("Paging file (free/total): %s/%s\n", infoBuf,
		GetHumanSize(statex.ullTotalPageFile, mem_human_sizes, 1024));
	strcpy_s(infoBuf, INFO_BUFFER_SIZE, GetHumanSize(statex.ullAvailVirtual, mem_human_sizes, 1024));
#if 0
	printf("Virtual memory (free/total): %s/%s\n", infoBuf,
		GetHumanSize(statex.ullTotalVirtual, mem_human_sizes, 1024));
	printf("Extended memory (free): %s\n",
		GetHumanSize(statex.ullAvailExtendedVirtual, mem_human_sizes, 1024));
#endif
}
