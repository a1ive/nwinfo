...
gnwinfo_draw_main_window(struct nk_context* ctx, float width, float height)
{
	if (!nk_begin_ex(ctx, "NWinfo GUI",
		...
		g_bginfo ? NK_WINDOW_BACKGROUND : (NK_WINDOW_BACKGROUND | NK_WINDOW_MINIMIZABLE | NK_WINDOW_CLOSABLE | NK_WINDOW_TITLE),
		GET_PNG(IDR_PNG_ICON), GET_PNG(IDR_PNG_MINIMIZE), GET_PNG(IDR_PNG_CLOSE)))
	{
		nk_end(ctx);
