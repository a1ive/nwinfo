...
gnwinfo_ctx_init(HINSTANCE inst, HWND wnd, struct nk_context* ctx, float width, float height)
{
	...
	g_ctx.main_flag2 = strtoul(gnwinfo_get_ini_value(L"Widgets", L"MainFlags2", L"0xFFFFFFFF"), NULL, 16);
	...
	nk_begin_ex(ctx, N_(N__LOADING),
		...
		NK_WINDOW_TITLE | NK_WINDOW_BORDER | NK_WINDOW_NO_INPUT, GET_PNG(IDR_PNG_REFRESH), nk_image_id(0), GET_PNG(IDR_PNG_CLOSE));
